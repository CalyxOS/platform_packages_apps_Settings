...
public class UserDetailsSettings extends SettingsPreferenceFragment
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        if (KEY_ENABLE_TELEPHONY.equals(preference.getKey())) {
            ...
            try {
                try {
                    getActivity().getPackageManager().getPackageInfoAsUser(
                            getActivity().getPackageName(), 0, mUserInfo.id);
                } catch (PackageManager.NameNotFoundException e) {
                    new PackageInstaller(AppGlobals.getPackageManager().getPackageInstaller(),
                            getActivity().getPackageName(), mUserInfo.id).installExistingPackage(
